<template>
    <div>
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list-wrap">
                        <div class="addr-list">
                            <ul>
                                <!-- 判断当前索引是否等于checkIndex,点击把当前索引赋值给checkIndex -->
                                <li v-for="(address, index) in addressListFilter" :class="{'check': checkIndex==index}" @click="checkIndex=index; selectAddId=address.addressId">
                                    <dl>
                                    ...
                                    <div class="addr-opration addr-del">
                                        <a href="javascript:;" class="addr-del-btn" @click="deleAddressConfirm(address.addressId)">
                                            <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                                        </a>
                                    </div>
                                    <div class="addr-opration addr-set-default">
                                        <a href="javascript:;" class="addr-set-default-btn" v-if="!address.isDefault" @click="defaultAddress(address.addressId)"><i>Set default</i></a>
                                    </div>
                    ...
                    <div class="next-btn-wrap">
                        <!-- 使用router-link实现路由跳转 -->
                        <router-link class="btn btn--m btn--red" v-bind:to="{path: 'orderConfirm', query: {'addressId': selectAddId}}">Next</router-link>
                    </div>
                </div>
            </div>
        </div>
        ...
        <modal v-bind:mdShow="isMdShow" v-on:close="closeModal">
            ...
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript:;" @click="deleAddress">确认</a>
...
export default {
    ...
    methods: {
        ...
        deleAddressConfirm(addressId){
            this.isMdShow = true
            this.addressId = addressId
        },
        deleAddress(){
            axios.post('/users/deleAddress', {addressId: this.addressId}).then((response) => {
                let res = response.data
                if(res.status == '0'){
                    this.isMdShow = false
                    this.init()
