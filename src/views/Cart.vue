<template>
    <div>
                            ...
                            <li v-for="item in cartList">
                                ...
                                <div class="cart-tab-5">
                                    <div class="cart-item-opration">
                                        <a href="javascript:;" class="item-edit-btn" @click="deleCartConfirm(item.productId)">
       ...
       <modal :mdShow="modalFlag" @click="closeModal">
            <p slot="message">确定是否删除</p>
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript:;" @click="deleCart">确认</a>
                <a class="btn btn--m" href="javascript:;" @click="modalFlag=false">关闭</a>
            </div>
        </modal>
    </div>
</template>
...
export default {
    ...
    //mounted只要组件加载完就会调用mounted
    mounted() {
        this.init()
    },
    //局部过滤器，在组件内部定义一个filters
    // filters:{
    //     currency: currency
    // },
    //计算属性
    //计算属性里的属性和data里的属性不能重复定义
    computed: {
        checkAllFlag(){
    ...
    methods: {
        ...
        deleCartConfirm(productId){
            this.modalFlag = true
            this.productId = productId
        },
        deleCart(){
            axios.post('/users/cartDele', {productId:this.productId}).then((response) => {
                let res = response.data
                if(res.status == '0') {
        ...
        selectAll(){
            //checkAllFlag是一个实时属性，只能在computed中赋值
            var flag = !this.checkAllFlag
            this.cartList.forEach((element) => {
                element.checked = flag ? '1' : '0'
            });
            axios.post('/users/cartEditAll', {checkAll: flag}).then((response) => {
                let res = response.data
                if(res.status == '0'){
        ...
        checkOut(){
            if(this.checkedCount > 0){
                //相当于window.history的封装。push()里面指定跳转页面
                this.$router.push({
                    path: '/address'
                })
            }
