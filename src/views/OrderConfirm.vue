                            ...
                            <li>
                                <div class="cart-tab-1">
                                    <div class="cart-item-pic">
                                        <img alt="item.productName" v-lazy="'/static/'+item.productImage">
                                    </div>
    ...
    methods:{
        init(){
            axios.get('/users/CartList').then((response) => {
                ...
                this.totalPrice = this.shipping - this.discount + this.tax + this.subTotal
            })
        },
        payMent(){
            var addressId = this.$route.query.addressId
            axios.post('/users/payMent', {orderTotal: this.totalPrice, addressId: addressId}).then((response) => {
                let res = response.data
                if(res.status == '0'){
                    this.$router.push({
